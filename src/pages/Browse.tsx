...
const Browse = () => {
  ...
  const filteredListings =
    listingsDb?.filter((listing) => {
      ...
      const matchesCity =
        ...
        selectedGenders
          .map((g) => g.toLowerCase())
          .includes(flatListing.preferences.gender.toLowerCase());
      
      

      const matchesRent =
        flatListing.rent.amount >= rentRange[0] &&
        flatListing.rent.amount <= rentRange[1];
  ...
  return (
    <div className="min-h-screen py-8 px-4 relative overflow-x-clip overflow-y-visible">
      <Landmark
        size={72}
        className="absolute left-12 bottom-[14%] text-emerald-400 drop-shadow-xl z-10 animate-float hidden md:block"
      />
      <Factory
        size={56}
        className="absolute right-20 bottom-[18%] text-pink-400 drop-shadow z-10 animate-float delay-250 md:block hidden"
      />
      <Warehouse
        size={38}
        className="absolute left-1/2 bottom-11 text-orange-300 drop-shadow-lg z-10 animate-float"
      />

      <div className="max-w-7xl mx-auto relative z-10">
        <BrowseHeader />

        <div className="mb-6 text-center text-slate-600 font-medium">
          Use the filters below to narrow down listings by city, preferred flatmate gender, and rent range.
        </div>
        ...
        <button
          ...
          onClick={() => {
            setSelectedGenders((prev) =>
              isSelected
                ? prev.filter((g) => g !== gender) // Deselect
                : [...prev, gender] // Select
            );
          }}
          className={`px-4 py-1 rounded-full border transition-all text-sm font-medium ${
            isSelected
        ...
        >
          ...
          <div className="w-full md:w-[300px]">
            ...
            <Slider
              ...
              trackStyle={{ backgroundColor: "hsl(213, 94%, 68%)" }}
              handleStyle={[
